...
class messenger:
	...
	def Return_Received_Text(self , Received_Json_From_Facebook) :
		text = Received_Json_From_Facebook["entry"][0]["messaging"][0].get("message")
		if text :  
			return text["text"] 
	...
	def Sent_text_Msgs(self , array_of_msgs , msg_type , rec_ID) :
		...
		for msg in array_of_msgs :
			...
			req = requests.post(post_message_url,  headers=headers, data=Json_Body ,timeout=3)	
	...
	def Sent_media_Msgs (self , array_of_msgs , rec_ID, media_type , Assest=False) :
		...
		for msg in array_of_msgs :
			...
			Json_Body["message"] = { "attachment":{ "type":media_type,  "payload":{ "url":msg,  "is_reusable": str(Assest).lower() }  } }
			Json_Body         = json.dumps(Json_Body)
			req = requests.post(post_message_url,  headers=headers, data=Json_Body , timeout=3)	
			Json_Body = json.loads(Json_Body)
	...
	def quick_reply ( self , text , array_of_quiks , rec_ID  ) :
		...
		Json_Body = {"recipient":{ "id":rec_ID }, "message":{"text": text, "quick_replies":array_of_quiks } } 
		Json_Body         = json.dumps(Json_Body)
		req = requests.post(post_message_url,  headers=headers, data=Json_Body ,timeout=3)
